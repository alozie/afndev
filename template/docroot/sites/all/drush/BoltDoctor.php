...
/**
 * @file
 * Defines class that powers `drush bolt-doctor` command.
 */
...
class BoltDoctor {
  ...
  protected function checkUriResponse() {
    $site_available = drush_shell_exec("curl -I --insecure %s", $this->baseUrl);
    if (!$site_available) {
      drush_log("Did not get response from $this->baseUrl", 'error');
  ...
  protected function checkHttps() {
    if (strstr($this->baseUrl, 'https')) {
      if (!drush_shell_exec('curl -cacert %s', $this->baseUrl)) {
  ...
  protected function checkDbConnection() {
    if (empty($this->statusTable['bootstrap']) || $this->statusTable['bootstrap'] != 'Successful') {
      ...
      drush_bolt_print_status_rows($this->statusTable, array(
        'db-driver',
        'db-hostname',
        'db-username',
        'db-password',
